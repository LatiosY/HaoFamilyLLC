...
export async function generateMetadata({
  ...
  return { title: dict.meta.sellTitle, description: dict.meta.sellDescription };
}
    ...
    <main className="bg-background">
      <section className="bg-navy px-5 pt-32 pb-16 text-white md:px-8 md:pt-40 md:pb-20">
        <div className="mx-auto max-w-6xl">
          ...
          <h1 className="font-serif mt-4 max-w-3xl text-4xl font-medium text-balance md:text-5xl">
            {dict.sell.title}
          </h1>
          <p className="mt-8 max-w-2xl text-sm leading-7 text-white/70 md:text-base">
            {dict.sell.intro}
          </p>
        </div>
      </section>
      <section className="mx-auto max-w-6xl px-5 py-16 md:px-8">
        <h2 className="font-serif text-3xl">{dict.sell.stepsTitle}</h2>
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-6xl px-5 py-16 md:px-8">
          <h2 className="font-serif text-3xl">{dict.sell.valuesTitle}</h2>
          <div className="mt-10 grid gap-10 md:grid-cols-3">
            {dict.sell.values.map((value) => (
              <div key={value.title}>
